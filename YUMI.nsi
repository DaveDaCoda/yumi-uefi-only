...
; EFI IOS
;  ${NSD_CreateCheckBox} 80% 43 100% 15 "Install EFI Bootable ISOs"
;  Pop $EFI
...
; Close All Open Explorer Windows 
  ...
  ;DetailPrint "Wiping any existing partition on $DestDisk using dd.exe. Please be patient." 
  ;nsExec::ExecToLog '"cmd" /c "echo y|$PLUGINSDIR\dd if=/dev/zero of=//./$DestDisk --size --progress"' ;/Q /y  
  ;DetailPrint "Wiping $DestDisk drive using dd.exe. Please be patient." 
  ;nsExec::ExecToLog '"cmd" /c "echo y|$PLUGINSDIR\dd if=/dev/zero od=$DestDisk --size --progress"' ;/Q /y   
...
;Function ISOEFI ; Set EFI ISO List
;  ${NSD_GetState} $EFI $EFICHECK
;  ${If} $EFICHECK == ${BST_CHECKED}
;  ${NSD_Check} $EFI
;    StrCpy $EFICHECK "Yes"
;  ${NSD_SetText} $EFI "Displaying EFI Bootable ISOs"
;    SendMessage $Distro ${CB_RESETCONTENT} 0 0 ; was ${NSD_LB_Clear} $Distro "" ; Clear all distro entries because a new format option may have been chosen ; Enable for DropBox
;	ShowWindow $Uninstaller 0 ; Disable Uninstaller option because we will be formatting the drive.
;   StrCpy $Checker "Yes"	
	
;  ${ElseIf} $EFICHECK == ${BST_UNCHECKED}
;  ${NSD_Uncheck} $EFI 
;  ${NSD_SetText} $EFI "Display EFI Bootable ISOs"  
;    SendMessage $Distro ${CB_RESETCONTENT} 0 0 ; was ${NSD_LB_Clear} $Distro "" ; Clear all distro entries because a new format option may have been chosen ; Enable for DropBox
;    ShowWindow $Uninstaller 1 ; Re-enable Uninstaller option.
;	StrCpy $Checker "Yes" 
;	Call SetSpace
