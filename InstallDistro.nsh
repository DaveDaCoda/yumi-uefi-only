...
; Ubuntu (New Method) 
 ...
 ${AndIf} ${FileExists} "$BootDir\multiboot\$JustISOName\$JustISO"  
 !insertmacro ReplaceInFile "linux /SLUG" "loopback loop /multiboot/$JustISOName/$JustISO$\r$\nlinux /multiboot/$JustISOName/casper/vmlinuz.efi iso-scan/filename=/multiboot/$JustISOName/$JustISO boot=casper noprompt floppy.allowed_drive_mask=0 ignore_uuid" "all" "all" "$BootDir\multiboot\$JustISOName\grub.cfg"  
 ${EndIf}
  
 ${If} ${FileExists} "$BootDir\multiboot\$JustISOName\casper\vmlinuz"
 ${AndIf} ${FileExists} "$BootDir\multiboot\$JustISOName\$JustISO"
 !insertmacro ReplaceInFile "linux /SLUG" "loopback loop /multiboot/$JustISOName/$JustISO$\r$\nlinux /multiboot/$JustISOName/casper/vmlinuz iso-scan/filename=/multiboot/$JustISOName/$JustISO boot=casper noprompt floppy.allowed_drive_mask=0 ignore_uuid" "all" "all" "$BootDir\multiboot\$JustISOName\grub.cfg"  
 ${EndIf}
...
; Windows 10
 ...
 ExecWait 'Dism /Split-Image /ImageFile:"$EXEDIR\TEMPYUMI\sources\install.wim" /SWMFile:"$EXEDIR\TEMPYUMI\sources\install.swm" /FileSize:3800'
 CopyFiles "$EXEDIR\TEMPYUMI\sources\*.swm" "$BOOTDIR\sources\"
